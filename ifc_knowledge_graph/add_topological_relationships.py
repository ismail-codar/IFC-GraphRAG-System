...
This script adds example topological relationships between existing nodes in the Neo4j database.
"""
...
def main():
    """Add topological relationships to the database."""
    try:
        # Connect to Neo4j
        conn = Neo4jConnector('neo4j://localhost:7687', 'neo4j', 'test1234')
        
        # 1. Add ADJACENT_TO relationships between walls
        logger.info("Adding ADJACENT_TO relationships between walls...")
        wall_adjacent_query = """
        MATCH (w1:Wall)
        MATCH (w2:Wall)
        WHERE w1 <> w2
        ...
        logger.info("Adding BOUNDS_SPACE relationships between storeys and walls...")
        bounds_query = """
        ...
        # 3. Add CONTAINS_TOPOLOGICALLY relationships
        logger.info("Adding CONTAINS_TOPOLOGICALLY relationships...")
        contains_query = """
        MATCH (s:IfcBuildingStorey)
        MATCH (e:Element) 
        WHERE NOT (e:Wall)
        ...
        # 4. Add CONNECTS_SPACES relationships for doors
        logger.info("Adding CONNECTS_SPACES relationships for doors...")
        connects_query = """
        ...
        logger.info("Adding IS_CONTAINED_IN relationships for windows...")
        window_query = """
        MATCH (w:Window)
        MATCH (wall:Wall)
        ...
        logger.error(f"Error adding topological relationships: {e}")
